...
export const JOB_DEFINITIONS = [
  ...
  // The four demo steps are NOT manual requeue choices: each needs a project
  // and an iteration in its payload, and the business card's own buttons
  // («Побудувати демо / заново», «Продовжити збірку», the critic's decisions)
  // start them with the right ones. A bare requeue from the «Інше…» dialog was
  // the «купа перезапусків кроків» nobody could tell apart (Roman, 2026-09-15).
  definition({ name: 'content-and-design', workerGroup: 'build', agentCapability: 'subscription', retry: DEFAULT_RETRY, expireInSeconds: AGENT_EXPIRY, schedulingClass: 'continuation', displayName: 'дизайн демо (тексти, стиль, сцени)', manualRequeue: false, payloadSchema: BUSINESS_PAYLOAD }),
  definition({ name: 'build-site', workerGroup: 'build', agentCapability: 'subscription', retry: { limit: 1, delaySeconds: 0 }, expireInSeconds: AGENT_EXPIRY, schedulingClass: 'continuation', displayName: 'збірка демо (код сайту)', manualRequeue: false, payloadSchema: BUSINESS_PAYLOAD }),
  definition({ name: 'visual-qa', workerGroup: 'build', agentCapability: 'subscription', retry: DEFAULT_RETRY, expireInSeconds: AGENT_EXPIRY, schedulingClass: 'continuation', displayName: 'перевірка демо критиком', manualRequeue: false, payloadSchema: BUSINESS_PAYLOAD }),
  definition({ name: 'deploy-demo', workerGroup: 'core', agentCapability: 'none', retry: DEFAULT_RETRY, expireInSeconds: STANDARD_EXPIRY, schedulingClass: 'continuation', displayName: 'публікація демо', manualRequeue: false, payloadSchema: BUSINESS_PAYLOAD }),
