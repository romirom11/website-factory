...
type WorkflowTransaction = Parameters<Parameters<WorkflowDatabase['transaction']>[0]>[0];
...
export async function transitionCurrentRun(
  ...
  if (!updated.length) {
    throw new Error(
      `workflow run ${attempt.runId} is no longer current for attempt ${attempt.attemptSequence}`,
    );
  }
}
